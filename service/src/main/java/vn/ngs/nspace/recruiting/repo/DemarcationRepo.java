...
public interface DemarcationRepo extends BaseRepo<Demarcation, Long> {
    ...
    @Query(value = "SELECT p.org_id as orgId, p.level_id as levelId, p.position_id as positionId, p.title_id as titleID, sum(p.sum_demarcation) as sumDemarcation FROM recruiting_service.demarcation as p\n" +
            "where p.status = 1 " +
    ...
    @Query(value = "SELECT p.id as id, p.org_id as orgId, p.level_id as levelId, p.position_id as positionId, p.title_id as titleIDs FROM recruiting_service.demarcation as p\n" +
            "where p.status = 1 " +
            "and (p.org_id = :orgId or coalesce(:orgId, 0) = 0)  " +
            "and (p.level_id = :levelId or coalesce(:levelId, 0) = 0) " +
            "and (p.position_id = :positionId or coalesce(:positionId, 0) = 0) " +
            "and (p.title_id = :titleId or coalesce(:titleId, 0) = 0) " +
            "and (extract(year from p.demarcation_date) = :dateDemarcation or coalesce(:dateDemarcation, 0) = 0) " +
            "and (extract(month from p.demarcation_date) = :dateDemarcationMonth or coalesce(:dateDemarcationMonth, 0) = 0) " +
            "", nativeQuery = true)
    List<Map<String, Object>> search(
            @Param("orgId") Long orgId
            ,@Param("levelId") Long levelId
            ...
            ,@Param("titleId") Long titleId
            ,@Param("dateDemarcation") Integer dateDemarcation
            ,@Param("dateDemarcationMonth") Integer dateDemarcationMonth
            );
