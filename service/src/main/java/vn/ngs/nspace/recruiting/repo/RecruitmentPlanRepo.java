...
public interface RecruitmentPlanRepo extends BaseRepo<RecruitmentPlan, Long> {
    ...
    @Query(value = " select s.* from recruiting_service.recruitment_plan s " +
            ...
            " and ((regexp_replace(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(lower(concat(coalesce(s.name, ''), coalesce(s.code, ''))), '[ớôốơợộồỗờợỡởổ]', 'o'), '[áàạảãăẳằặẵắầẩẫậấâ]', 'a'),'[ểệềếê]','e'),'[ứửựừư]','u'),'[ỉịìĩí]','i')) like lower(concat('%', :search\\:\\:varchar , '%')) or coalesce(:search\\:\\:varchar, '#') = '#')"+
